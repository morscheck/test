...
class Instagram
{
    ...
    public function checkUser($username){
        $data = $this->curl('https://www.instagram.com/'.$username);
        $hsl = preg_match('/window._sharedData = (.*?);<\/script>/', $data[1], $hsl) ? $hsl[1] : null;
        $json = json_decode($hsl);
    ...
    public function ViewFollowTarget() {
        ...
        if($status->status == 'ok'){
            ...
            echo " -> 2. ".$this->lightcyan."Following".$this->white."\n";
